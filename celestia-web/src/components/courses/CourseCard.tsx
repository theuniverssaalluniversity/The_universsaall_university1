...
const CourseCard = ({ title, description, thumbnail_url, price, slug }: CourseCardProps) => {
    return (
        <Link to={`/courses/${slug}`}>
            <motion.div
                whileHover={{ y: -5 }}
                className="group bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden hover:border-primary/50 transition-all duration-300 h-full flex flex-col"
            >
                ...
                <div className="p-6 flex-1 flex flex-col">
                    ...
                    <div className="flex items-center justify-between mt-auto">
                        ...
                        <div className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center text-white group-hover:bg-primary group-hover:text-black group-hover:border-primary transition-all">
                            <ArrowRight size={18} />
                        </div>
                    </div>
                </div>
